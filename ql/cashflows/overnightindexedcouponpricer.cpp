...
namespace QuantLib {
    ...
    std::tuple<Rate, Spread, Rate> CompoundingOvernightIndexedCouponPricer::compute(const Date& date) const {
        ...
        if (i < n) {
            ...
            QL_REQUIRE(curve->referenceDate() <= valueDates[i] && (curve->allowsExtrapolation()
                       || valueDates[n] <= curve->maxDate()),
            ...
            if (coupon_->canApplyTelescopicFormula()) {
                // handle partial accrual of first fixing if accrualStartDate lands on a fixing holiday
                const Size telescopicStartIdx = (
                    i == 0 && !applyObservationShift && (valueDates.front() < coupon_->accrualStartDate())) ? 1: i;
        ...
        const auto [rateAccrualStartDate, rateAccrualEndDate] = applyObservationShift
            ...
            : std::make_pair(coupon_->accrualStartDate(), std::min(date, coupon_->accrualEndDate()));
