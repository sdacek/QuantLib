...
namespace QuantLib {
    ...
    : FloatingRateCoupon(paymentDate, nominal, startDate, endDate,
        ...
        valueDates_ = fixingCal.businessDayList(
            fixingCal.adjust(startDate, Preceding),
        ...
        if (telescopicValueDates) {
            ...
            tmpEndDate = fixingCal.adjust(endDate, Following);
            const Date tmpLockoutDate = fixingCal.advance(endDate,
        ...
        interestDates_.front() = std::max(interestDates_.front(), startDate);
        interestDates_.back() = std::min(interestDates_.back(), endDate);
        ...
        if (lockoutDays_ != 0) {
            ...
            std::fill(fixingDates_.end() - lockoutDays_ - 1, fixingDates_.end(), lockoutDate);
        }
